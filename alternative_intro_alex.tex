...
Let $S$ be an approximate homomorphic-encryption scheme satisfying the scheme, correctness, noise-coordinate, and IND-CPA interfaces summarized in Table~\ref{tab:theorem-boundary}. Let $\operatorname{NF}*{\gamma}[S]$ denote its noise-flooded transform, where $\gamma>0$ is the Gaussian-width multiplier, and let $n$ be the dimension of the integer noise-coordinate representation. For every well-typed IND-CPAD adversary $\mathcal{A}$ and every decryption-query budget $q$, the formal development constructs an IND-CPA adversary $\mathcal{B}*{\mathcal{A},q}$ and proves
\begin{equation}
\begin{aligned}
&\operatorname{WinProb}!\left[
\operatorname{IND\text{-}CPAD}*{
\operatorname{NF}*{\gamma}[S],q
}(\mathcal{A})
\right]
\
&\qquad\leq
\beta_{\mathrm{CPA}}!\left(
\mathcal{B}*{\mathcal{A},q}
\right)
+
\frac{\sqrt{qn}}{2\gamma}.
\end{aligned}
...
\end{equation}
Here $\beta*{\mathrm{CPA}}(\mathcal{B}*{\mathcal{A},q})$ is the supplied IND-CPA winning-probability bound for the underlying scheme. Under the convention that advantage is excess winning probability over $1/2$,
$$
...
\]
The exported Rocq result is \texttt{NoiseFloodingSecure.is_secure}.
...
The checked result deliberately isolates the good-execution core of the LMSS argument. It assumes deterministic decryption, probability-one---or support-level---approximate correctness, and the integer-coordinate interface stated in Table~\ref{tab:theorem-boundary}. A concrete CKKS instantiation would additionally require an up-to-bad game hop accounting for nonzero correctness error, as well as proofs that the chosen CKKS instantiation satisfies the scheme, coordinate, and IND-CPA assumptions. We do not claim a verified CKKS implementation or a concrete CKKS parameter set.
...
Section~\ref{sec:construction} defines the noise-flooded construction and the abstract assumptions used by the theorem. The complete IND-CPA and IND-CPAD experiments and reduction pseudocode appear in Appendix~\ref{app:security-games}. Section~\ref{sec:reduction} presents the cryptographic game sequence and isolates the adaptive statistical hop. Section~\ref{sec:program-logic} develops the Pythagorean judgment, its probability-theoretic foundation, the trace compiler, and the generic local-to-adaptive theorem, and then instantiates them to close the noise-flooding reduction. Section~\ref{sec:artifact} describes the artifact, theorem-audit path, and trusted base. Section~\ref{sec:related-work} discusses related work, and Section~\ref{sec:conclusion} concludes.
