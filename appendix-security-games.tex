...
\begin{algorithmic}
  ...
  \Procedure{OEval}{$f,\mathcal{I}$}
    \State $m_0' \gets f((T_i.m_0)_{i\in\mathcal{I}})$
    \State $m_1' \gets f((T_i.m_1)_{i\in\mathcal{I}})$
    \State $c' \gets Eval(f,(T_i.c)_{i\in\mathcal{I}})$
    \State $T \gets T \doubleplus [(m_0',m_1',c')]$
    \State \Return $c'$
  \EndProcedure
  \State
  \Procedure{ODec}{$i$}
    ...
    \If{$T_i.m_0\ne T_i.m_1$}
      \State \Return $\bot$
    \Else
      \State \Return a flooded plaintext sample according to the current game
    \EndIf
  \EndProcedure
\end{algorithmic}
\end{algorithm}
...
\begin{algorithmic}[1]
  ...
  \Procedure{$OEval_{\mathcal{B}}$}{$f,\mathcal{I}$}
    \State Compute $m_0',m_1'$ from the rows indexed by $\mathcal{I}$.
    \State $c' \gets Eval(f,(T_i.c)_{i\in\mathcal{I}})$
    \State $T \gets T \doubleplus [(m_0',m_1',c')]$
    \State \Return $c'$
  \EndProcedure
