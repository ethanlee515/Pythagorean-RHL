...
This appendix records the game syntax suppressed in \Cref{sec:verified-reduction}.
It is not meant to be a second proof outline; the main text reasons about the named Coq games and their hybrid relations.
...
The real game returns a noise-flooded sample centered at $Dec(sk,T_i.c)$.
The simulated-decrypt game returns a noise-flooded sample centered at the stored plaintext $T_i.m_b$.
The formal proof factors the deterministic prefix of \textsc{ODec} from this last noisy continuation, which is why the one-call replacement theorem can focus only on the two Gaussian centers.
...
\begin{algorithmic}[1]
  \Procedure{$\mathsf{Flood}$}{$m,e$}
    \State $\eta \gets D_{\bbZ^n,\max\{1,e\}\gamma}$
...
This is harmless for decryption queries that return a value: the simulator only returns a flooded plaintext when the stored labels agree, $m_0=m_1$, so there is no bit-dependent plaintext choice left to make.
