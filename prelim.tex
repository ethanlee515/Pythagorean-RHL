...
Distance and charts & Dimension \(n\) and the three laws in
\Cref{eq:charts} & Turns message distance into integer-coordinate
displacement \\
...
LMSS state approximate correctness using a message space embedded in a normed
space~\cite{lmss}.
The following smaller distance-and-chart interface is specific to our
formalization.
The message space carries a distance
\(d:M\times M\to\bbN\).
For each center \(a\in M\), there are chart maps
\[
...
\(J_a(v)\) applies displacement \(v\) at \(a\).
The laws say that these coordinates are centered, preserve distance, and
change base point by translation.
...
\end{aligned}
\end{equation}
Here the tuple Gaussian has \(n\) independent integer coordinates.
...
gives
\(d(\mathsf{dec}_0(sk,c),m)\leq e\).
By \Cref{eq:charts}, the two Gaussian centers differ by at most \(e\) in every
coordinate.
...
\label{thm:main}
Let \(S\), its distance-and-chart interface, probability-one correctness proof,
