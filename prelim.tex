...
\]
These algorithms are distribution-valued, and key generation is lossless: it
returns with probability one.
...
\]
Adversaries are well-typed SSProve oracle programs.  Running time is not part
of the semantics, so preservation of probabilistic polynomial time by the
explicit reduction remains a separate inspection~\cite{ssprove}.
...
Our contribution is to express its assumptions through the interfaces in
\Cref{sec:construction} and check the resulting adaptive reduction in Rocq.
...
the requirements of \Cref{sec:construction}.
